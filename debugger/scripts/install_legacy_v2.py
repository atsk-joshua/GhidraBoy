...
if not args.ghidra:raise SystemExit('Set GHIDRA_INSTALL_DIR or pass --ghidra /path/to/ghidra_12.1.3_PUBLIC')
...
if properties.get('application.version')!='12.1.3':raise SystemExit('This package requires Ghidra 12.1.3')
home=args.user_home.resolve()
if platform.system()=='Darwin':settings=home/'Library/ghidra/ghidra_12.1.3_PUBLIC';suffix='dylib'
elif platform.system()=='Linux':settings=home/'.config/ghidra/ghidra_12.1.3_PUBLIC';suffix='so'
else:raise SystemExit('Target Linux x86-64 or development macOS arm64 required')
archives=[Path(os.environ.get('GHIDRABOY_ZIP',str(ROOT.parent/'GhidraBoy/build/distributions/ghidra_12.1.3_PUBLIC_20260905-suite1_GhidraBoy.zip'))), Path(os.environ.get('GHIGBC_ZIP',str(ROOT/'ghidra-extension/dist/ghidra_12.1.3_PUBLIC_20260905_GhiGBC.zip')))]
...
manifest={'schema':2,'ghidra':'12.1.3','source':str(ROOT),'runtime':str(runtime),'entries':[]}
def record(entry):
...
try:
    ...
    for relative in (f'build/libghigbc.{suffix}','build/teaching.gbc','.deps/SameBoy/build/bin/BootROMs/cgb_boot.bin','native/patches/0001-cpu-bus-provenance.patch','dependencies.lock.json','LICENSE','scripts/doctor.py','scripts/install.py'):
        copy_file(relative)
