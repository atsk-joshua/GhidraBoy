...
p.add_argument('--library',type=Path,default=root/'build/libghigbc.so')
p.add_argument('--output',type=Path,default=root/'build/reports/linux-library.json')
p.add_argument('--compiler',default='unspecified')
options=p.parse_args();file=options.library;data=file.read_bytes()
...
report={'format':'ELF64 little-endian shared object','architecture':'x86-64','target':'linux-x86_64','compiler':options.compiler,'sha256':hashlib.sha256(data).hexdigest(),'required_libraries':needed,'symbol_versions':sorted(set(versions)),'abi_exports_verified':sorted(required),'linux_execution_verified':False,'steam_deck_desktop_verified':False}
