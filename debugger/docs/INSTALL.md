...
Run the commands on this page from the root of an extracted platform archive. Keep Ghidra closed while changing its extensions. Use the existing system Python3.9+ (including the Python already on Steam Deck), Ghidra **12.1.3**, and Java21. No compiler, Gradle, RGBDS, root access, SteamOS read-only change, or global JVM library-path change is required.
...
the matching platform-independent companion (included when listed in suite.json) to a new copy of the distribution
...
For an optional compatible study package, extract it separately and add
`--study "/path/to/study-package/study.json"`. The installer verifies the matched
composition and all payload hashes before replacing an extension. Follow that
package's own instructions for its supported static-only and live components.
