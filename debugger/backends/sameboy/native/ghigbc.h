...
enum { GC_MODEL_CGB_E=0, GC_MODEL_DMG_B=1 };
typedef struct {
    uint32_t api, model, cgb_mode, wram_size, vram_size, boot_size;
} gc_hardware;
...
/* Additive model API; the original entry points retain CGB-E behavior. */
gc_machine *gc_create_model_buffers(const uint8_t *rom, uint32_t rom_size,
                                    const uint8_t *boot, uint32_t boot_size, uint32_t model);
...
/* State, memory, events and hardware describe the same locked instruction boundary. */
int gc_snapshot_hardware(gc_machine *m, gc_state *state, uint8_t *memory, uint32_t size,
                         gc_event *events, uint32_t capacity, gc_hardware *hardware);
