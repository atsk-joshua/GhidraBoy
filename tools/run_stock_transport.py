...
def main(a):
    ...
    def run(name,project,input,script,args,readonly=False):
        command=common+[project]+(['-process',input,'-readOnly'] if readonly else ['-import',str(input),'-processor','SM83:LE:16:default'])+['-noanalysis','-scriptPath',str(REPO/'src/test/scripts')+';'+str(a.ghidra/'Ghidra/Extensions/GhidraBoy/ghidra_scripts'),'-postScript',script,str(e/name),*args]
    ...
    if a.phase in ('setup','all'):
        for name,project,fixture,script,args in [
            ('g2','G2',REPO/'src/test/resources/ordinary/PREDICATED_CALLS.gb','GhidraBoyStockFaults.java',[]),
            ('loop','Loop',e/'fixtures/W3B_LOOP.gb','GhidraBoyStockFaults.java',['loop']),
            ('ordinary','Ordinary',REPO/'src/test/resources/ordinary/F1234.gb','GhidraBoyW2eFinite.java',[]),
            ('memory','Memory',e/'fixtures/W4_MEMORY_IMAGE.gb','GhidraBoyMemoryImages.java',['memory']),
            ('source','Source',e/'fixtures/W4_MEMORY_IMAGE.gb','GhidraBoyMemoryImages.java',['source']),
            ('image','Current',e/'fixtures/W4_MEMORY_IMAGE.gb','GhidraBoyMemoryImages.java',['image']),
            ('domains','Current',e/'fixtures/W3B_DOMAINS.gb','GhidraBoyW3bDomains.java',['persist-canonical']),
            ('controls-samebytes','SameBytes',e/'fixtures/W4_MEMORY_IMAGE.gb','GhidraBoyMemoryImages.java',['samebytes']),
            ('controls-interference','Interference',e/'fixtures/W4_MEMORY_IMAGE.gb','GhidraBoyMemoryImages.java',['interference']),
            ('order','Order',e/'fixtures/W3B_ORDER.gb','GhidraBoyW3bOrder.java',[])]:run(name,project,fixture,script,args)
        run('damage','G2','PREDICATED_CALLS.gb','GhidraBoyStockDamage.java',[],True)
    if a.phase in ('reopen','all'):
        ...
        run('image-reopen','Current','W4_MEMORY_IMAGE.gb','GhidraBoyMemoryImages.java',['reopen'],True)
        run('domains-reopen','Current','W3B_DOMAINS.gb','GhidraBoyW3bDomains.java',['reopen','public',str(e/'domains')],True)
