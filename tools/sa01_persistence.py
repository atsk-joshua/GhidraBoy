#!/usr/bin/env python3
"""Reproduce the generic SA-01 baseline and three-process saved-result checks.
...
def main():
    ...
    fixture = work / "sa01-generic-mbc3-v1.gb"
    ...
    records = []
    for mode in ("prepare", "reopen", "edited"):
        command = [str(ghidra / "support/analyzeHeadless"), str(projects), "sa01"]
        ...
        command += ["-noanalysis", "-scriptPath", str(repo / "src/test/scripts"),
                    "-postScript", "GhidraBoySa01.java", mode, str(work), digest]
        ...
        marker = {"prepare": "SA01_PREPARE_PASS", "reopen": "SA01_REOPEN_REAPPLY_PASS",
                  "edited": "SA01_EDITED_REOPEN_PASS"}[mode]
        ...
        if not passed:
            raise SystemExit(1)
    print("SA01_REVIEW_PERSISTENCE_PASS")
