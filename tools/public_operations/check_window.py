...
def check_request_ordering(root,timeline=None):
    ...
    for switched in (False,True):
        ...
        for outcome in (a,later):
            ...
            core.require(d['arguments']==['conditional-call-explain',outcome['entry']],'wrong requested domain')

        expected='ACTIVATION_SUPERSEDED' if switched else 'REQUEST_SUPERSEDED'
...
def ordering_sensitivities(root):
    import copy
    original=core.read(root/'timeline.json');check_request_ordering(root,original);results={}
    ...
    for name,change in changes.items():
        ...
        try:check_request_ordering(root,trace)
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('root',type=Path);p.add_argument('--label');p.add_argument('--ordering',action='store_true');p.add_argument('--delta',type=int,choices=[-1,1],default=1);p.add_argument('--output',type=Path,required=True);a=p.parse_args()
    if a.ordering:result=dict(scope='ACTUAL_PUBLIC_REQUEST_ORDERING',rows=check_request_ordering(a.root),sensitivities=ordering_sensitivities(a.root))
