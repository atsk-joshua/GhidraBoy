...
public class W2NativeObserver {
  ...
  public static void main(String[] args)throws Exception {
    ...
    for(String name:List.of("ghidra.app.decompiler.DecompInterface","ghidra.app.decompiler.DecompileResults"))for(var type:vm.classesByName(name))installEntry(manager,type);
    ...
    append(out,"observer-installed\t"+System.nanoTime()+"\tpublic-decompileFunction-entry-return-breakpoints; public-result-constructor; provider exceptions; suspend-event-thread; callback-public-support-method; no private field reads");
    vm.resume();
    try {while(true){var set=vm.eventQueue().remove();try{
      for(var event:set) {
        if(event instanceof ClassPrepareEvent e){installEntry(manager,e.referenceType());}
          ...
          append(out,"provider-exception\t"+System.nanoTime()+"\t"+(request==null?"UNMATCHED":request.id())+"\tthread="+e.thread().uniqueID()+"\texception_object="+e.exception().uniqueID()+"\tlocation="+e.location()+"\tmessage="+message.value().replace('\n',' '));
