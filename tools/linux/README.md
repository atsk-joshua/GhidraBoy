...
`runtime-check.sh` consolidates these checks for each extracted backend selection. Mount its package at `/candidate`, the copied Ghidra distribution at `/ghidra` (read-only), a fresh result directory at `/out`, and the script at `/runner.sh` (read-only). Run `xvfb-run -a bash /runner.sh` as an unprivileged user with `--init --network none --hostname ghidraboy-linux --add-host ghidraboy-linux:127.0.0.1`. Mount neither the checkout nor a compiler tree. The display smoke checks every backend selected by the package and requires packaged SDL.

The optional `NativeAdapters.Dockerfile` derives a separate CMake-capable builder from the retained native compiler image for the pinned mGBA adapter. Verify its base tag resolves to the M3 `builderImage` above before building; record the resulting image ID because package repositories can change. `debugger/scripts/build_mgba.py --source SOURCE_TAR --work NEW_DIRECTORY --output ISOLATED_NATIVE_DIRECTORY` produces the adapter, pristine corresponding source tar and build receipt. Repeating this in another fresh directory verifies binary and canonical build-identity reproduction. Select these outputs with `package_candidate.py --platform linux-x86_64 --native-dir ISOLATED_NATIVE_DIRECTORY --backend sameboy|mgba|both`. See `../../docs/integration/m8-linux-notes.md` for the executed qualification tuple.
