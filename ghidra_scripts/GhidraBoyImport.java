...
public class GhidraBoyImport extends GhidraScript {
    @Override public void run() throws Exception {
        ...
        String mode=args.length>1?args[1]:askChoice("Input mode","Explicit layout",List.of("CARTRIDGE","SALVAGE","DMG_BOOT","CGB_BOOT"),"CARTRIDGE");
        String override=args.length>2?args[2]:"AUTO";
        var lang=getLanguage(new LanguageID("SM83:LE:16:default"));
        if(Files.size(input)>(mode.equals("SALVAGE")?0x1000000:0x800000)) throw new IllegalArgumentException("Input exceeds 8 MiB limit");
        ...
        try(var provider=new ByteArrayProvider(Files.readAllBytes(input))) {
            var kind=args.length>3?GameBoyKind.valueOf(args[3]):isRunningHeadless()?GameBoyKind.CGB:askChoice("Hardware", "Explicit hardware selection", List.of(GameBoyKind.GB,GameBoyKind.CGB),GameBoyKind.CGB);
