...
public class GhidraBoyAbi extends GhidraScript {
  ...
  public void run() throws Exception {
    ...
    Path input =
        args.length > 0
            ? Path.of(args[0])
            : askFile("Per-function ABI request JSON", "Preview").toPath();
    var at =
        args.length > 1 ? currentProgram.getAddressFactory().getAddress(args[1]) : currentAddress;
    ...
    boolean apply =
        isRunningHeadless()
            ? args.length > 2 && args[2].equals("apply")
            : askYesNo(
                "Apply ABI", "Apply this storage and cleanup to the selected function only?");
