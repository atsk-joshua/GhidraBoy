...
set -euo pipefail
cd "$(dirname "$0")/.."
: "${GHIDRA_INSTALL_DIR:?Set Ghidra 12.1.2 path}"
...
python3 scripts/install.py --user-home "$ui_home"
...
settings = Path(sys.argv[2]) / ('Library/ghidra' if platform.system() == 'Darwin' else '.config/ghidra') / 'ghidra_12.1.2_PUBLIC/Extensions'
...
if [[ "${1:-}" == "--hp" ]]; then ui_test=HpWatchUiTest; fi
"$JAVA_HOME/bin/javac" -proc:none -cp "$ui_classpath" -d "$ui_classes" tests/ghidra/UiActionTest.java tests/ghidra/HpWatchUiTest.java
