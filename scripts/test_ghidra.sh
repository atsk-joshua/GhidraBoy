...
python3 scripts/install.py --user-home "$user_home"
...
settings=home/('Library/ghidra' if platform.system()=='Darwin' else '.config/ghidra')/'ghidra_12.1.2_PUBLIC/Extensions'
...
"$JAVA_HOME/bin/javac" -proc:none -cp "$cp_path" -d "$test_classes" tests/ghidra/RealTraceTest.java
