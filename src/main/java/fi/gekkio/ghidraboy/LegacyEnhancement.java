...
public final class LegacyEnhancement {
    ...
    public static ProgramMapping.Snapshot enhance(Program p,String mapperOverride,GameBoyKind hardware,TaskMonitor monitor) throws Exception {
        ...
        if(existing!=null) {
            if(hardware!=null) {
                int id=p.startTransaction("Explicit legacy hardware identification"); boolean done=false;
                try {
                    p.getOptions(ProgramMapping.OPTIONS).setString("cartridge",ProgramMapping.JSON.toJson(existing.withHardware(hardware)));
                    p.getOptions(ProgramMapping.OPTIONS).setString("hardware",hardware.name());
                    p.getOptions(ProgramMapping.OPTIONS).setString("hardwareProvenance","explicit enhancement choice");
                    monitor.checkCancelled(); done=true;
                } finally { p.endTransaction(id,done); }
            }
        ...
        var descriptor=Cartridge.parse(bytes,mapperOverride).withHardwareChoice(hardware==null?historical:hardware.name());
        ...
        try {
            ...
            opts.setString("hardwareProvenance",hardware!=null?"explicit enhancement choice":descriptor.hardwareKnown()?"persisted historical choice":"not recovered; header not authoritative");
