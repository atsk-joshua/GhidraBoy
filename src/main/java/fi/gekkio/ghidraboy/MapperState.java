...
public record MapperState(
  ...
  public MapperState {
    if (romLow < 0
        ...
        || svbk > 7
        || latch < 0
        || latch > 255) throw new IllegalArgumentException("Mapper state register out of range");
    if (svbk == 0) svbk = 1;
  }

  public static MapperState reset() {
    return new MapperState(1, 0, 0, 0, false, 0, 1, 0);
  }
  ...
  public boolean rtcLatchRequested(Cartridge c, int address, int value) {
    return c.mapper() == Cartridge.Mapper.MBC3
        && c.rtc()
        && address >= 0x6000
        && address < 0x8000
        && latch == 0
  ...
  public static int ramReadValue(Cartridge c, int stored) {
    return c.mapper() == Cartridge.Mapper.MBC2
        ? 0xf0 | ramWriteValue(c, stored)
        : ramWriteValue(c, stored);
  }
  ...
  public record Resolution(String status, Physical physical, String reason) {
    static Resolution mapped(String r, int b, int o) {
      return new Resolution("mapped", new Physical(r, b, o), "");
    }

    static Resolution other(String s, String why) {
      return new Resolution(s, null, why);
    }
  }
  ...
  public static Resolution translate(Cartridge c, MapperState s, int cpu, boolean write) {
    ...
    if (!c.hardwareKnown()
        && ((cpu >= 0x8000 && cpu < 0xa000)
            || (cpu >= 0xd000 && cpu < 0xe000)
            || (cpu >= 0xf000 && cpu < 0xfe00)))
      return Resolution.other("unknown", "Historical GB/CGB hardware selection is unknown");
    if (cpu < 0x8000) {
      if (write)
        return Resolution.other(
            c.mapper() == Cartridge.Mapper.ROM_ONLY ? "unmapped" : "device",
            "ROM is read-only; mapper control write");
      if (c.mapper() == Cartridge.Mapper.RAW)
        return Resolution.other("unknown", "Unsupported mapper");
      if (s == null
          && cpu < 0x4000
          && (c.mapper() == Cartridge.Mapper.MBC2
              || c.mapper() == Cartridge.Mapper.MBC3
              || c.mapper() == Cartridge.Mapper.MBC5)) return Resolution.mapped("ROM", 0, cpu);
      if (c.mapper() != Cartridge.Mapper.ROM_ONLY && s == null)
        return Resolution.other("unknown", "Explicit mapper state required");
      int bank = cpu / 0x4000;
      switch (c.mapper()) {
        case MBC1 ->
            bank =
                cpu < 0x4000
                    ? (s.mode == 0 ? 0 : s.romHigh << 5)
                    : (s.romHigh << 5) | (s.romLow == 0 ? 1 : s.romLow);
      ...
      if ((banks & (banks - 1)) != 0)
        return Resolution.other("unknown", "Non-power-of-two ROM wiring is not inferred");
    ...
    if (cpu < 0xc000) {
      if (c.mapper() == Cartridge.Mapper.RAW)
        return Resolution.other("unknown", "Unsupported external device");
      if (c.mapper() != Cartridge.Mapper.ROM_ONLY && s == null)
        return Resolution.other("unknown", "RAM selection required");
      if (c.mapper() != Cartridge.Mapper.ROM_ONLY && !s.ramEnabled)
        return Resolution.other("unmapped", "RAM/RTC disabled");
      if (c.mapper() == Cartridge.Mapper.MBC3 && s.ramSelect >= 8 && s.ramSelect <= 12)
        return Resolution.other(
            c.rtc() ? "device" : "unmapped", "MBC3 RTC register selection (no clock model)");
      if (c.mapper() == Cartridge.Mapper.MBC3 && s.ramSelect > 3)
        return Resolution.other("unmapped", "Invalid RAM/RTC selection");
    ...
    if (cpu < 0xff80 || cpu == 0xffff)
      return Resolution.other("device", "Volatile hardware register");
