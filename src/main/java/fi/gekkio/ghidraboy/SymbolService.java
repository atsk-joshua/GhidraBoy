...
public final class SymbolService {
  ...
  public static List<Placement> preview(Program p, SymbolFile.Result parsed) throws IOException {
    ...
    for (var symbol : parsed.symbols()) {
      var l = symbol.location();
      var found = new LinkedHashSet<Address>();
      String reason = "";
      if (l.address() == 0x8000
          || l.address() == 0xa000
          || l.address() == 0xc000
      ...
      if (found.isEmpty() && reason.isEmpty())
        reason = "Unmapped or unsupported symbol location; source retained";
      out.add(new Placement(symbol, List.copyOf(found), reason));
    }
    return List.copyOf(out);
  }

  public static String entryKey(SymbolFile.Symbol symbol) {
    return SymbolFile.format(List.of(symbol)).strip();
  }

  public static List<Address> boundaryCandidates(Program p, SymbolFile.Symbol symbol)
      throws IOException {
    var snapshot = ProgramMapping.inspect(p);
    var out = new TreeSet<Address>();
    int cpu = symbol.location().address();
    for (var range : snapshot.ranges()) {
      ...
      if (range.start() == cpu || range.start() + range.length() == cpu) {
        ...
        for (var physical : ProgramMapping.staticToPhysical(p, space.getAddress(probe), snapshot))
          if (matches(symbol.location(), physical, snapshot.cartridge()))
            out.add(space.getAddressInThisSpaceOnly(cpu));
      }
    }
    return List.copyOf(out);
  }
  ...
  public static Optional<java.nio.file.Path> companion(Program p) {
    ...
    try {
      ...
      return java.nio.file.Files.isRegularFile(candidate)
  ...
  private static boolean matches(
      SymbolFile.Location location, MapperState.Physical physical, Cartridge cartridge) {
    boolean boot = physical.region().equals("BOOT");
    long bank = physical.bank();
    if (cartridge != null
        && cartridge.mapper() == Cartridge.Mapper.ROM_ONLY
        && physical.region().equals("ROM")) bank = 0;
  ...
  private static final class LabelIdentity {
    ...
    LabelIdentity(ghidra.program.model.symbol.Symbol symbol, boolean created) {
      id = symbol.getID();
      name = symbol.getName();
      qualifiedName = symbol.getName(true);
      namespace = symbol.getParentNamespace().getID();
      space = symbol.getAddress().getAddressSpace().getSpaceID();
    ...
    boolean unchanged(ghidra.program.model.symbol.Symbol symbol) {
      return symbol != null
          ...
          && symbol.getName(true).equals(qualifiedName)
          && symbol.getParentNamespace().getID() == namespace
          && symbol.getAddress().getAddressSpace().getSpaceID() == space
          && symbol.getAddress().getOffset() == offset;
    }
  }
    ...
    for (String key : opts.getOptionNames())
      if (key.startsWith("symbols.") && key.endsWith(".source")) {
        ...
        for (var placement : preview(p, parsed))
          for (var address : placement.addresses()) {
            var symbol = existing(p, address, placement.symbol().name());
            if (symbol == null) continue;
            var identity =
                registry.labels.computeIfAbsent(
                    symbol.getID(), id -> new LabelIdentity(symbol, previouslyOwned.contains(id)));
  ...
  private static ghidra.program.model.symbol.Symbol existing(
      Program p, Address address, String name) {
  ...
  private static void collectUnclaimed(Program p, Registry registry, TaskMonitor monitor)
      throws Exception {
  ...
  public static List<Source> sources(Program p) throws IOException {
    ...
    return registry.sources.entrySet().stream()
        .map(
            entry ->
                new Source(
                    entry.getKey(),
                    entry.getValue(),
                    (int)
                        registry.labels.values().stream()
                            .filter(label -> label.claims.contains(entry.getKey()))
                            .count()))
        .toList();
  }

  public static List<Placement> importSymbols(
      Program p, SymbolFile.Result parsed, String source, TaskMonitor monitor) throws Exception {
    return importSymbols(p, parsed, source, Map.of(), monitor);
  }

  public static List<Placement> importSymbols(
      Program p,
      SymbolFile.Result parsed,
      String source,
    ...
    var choices =
        new TreeMap<String, AnalysisOwnership.Point>(
            registryBefore.placements.getOrDefault(source, Map.of()));
    for (var entry : overrides.entrySet()) {
      var address = ProgramMapping.staticAddress(p, entry.getValue());
      if (address == null)
        throw new IllegalArgumentException("Unknown static address " + entry.getValue());
    ...
    for (var placement : preview(p, parsed)) {
      ...
      if (selected != null) {
        ...
        if (!boundaryCandidates(p, placement.symbol()).contains(address))
          throw new IllegalArgumentException(
              "Stale or invalid boundary placement for " + entryKey(placement.symbol()));
        placement =
            new Placement(
                placement.symbol(), List.of(address), "Explicit boundary/end-marker placement");
      }
      placements.add(placement);
    }
    monitor.checkCancelled();
    int tx = p.startTransaction("Import Game Boy symbols");
    ...
    try {
      ...
      for (var placement : placements)
        for (var address : placement.addresses()) {
          ...
          if (created)
            symbol =
                p.getSymbolTable()
                    .createLabel(address, placement.symbol().name(), SourceType.IMPORTED);
          var identity = registry.labels.get(symbol.getID());
          if (identity == null) {
            identity = new LabelIdentity(symbol, created);
            registry.labels.put(symbol.getID(), identity);
          }
          identity.claims.add(source);
        }
      ...
      p.getOptions(ProgramMapping.OPTIONS)
          .setString(REGISTRY, ProgramMapping.JSON.toJson(registry));
      monitor.checkCancelled();
      success = true;
    } finally {
  ...
  public static void removeOwned(Program p, String source, TaskMonitor monitor) throws Exception {
    int tx = p.startTransaction("Remove Game Boy symbol source");
    boolean success = false;
    try {
      var registry = registry(p);
  ...
  public static Export exportSymbols(Program p, TaskMonitor monitor) throws Exception {
    var out = new ArrayList<SymbolFile.Symbol>();
    ...
    while (symbols.hasNext()) {
      ...
      if (symbol.getSource() == SourceType.DEFAULT
          || (!symbol.getSymbolType().equals(ghidra.program.model.symbol.SymbolType.LABEL)
              && !symbol.getSymbolType().equals(ghidra.program.model.symbol.SymbolType.FUNCTION)))
        continue;
      ...
      if (identities.size() != 1) {
        ...
        for (var choices : registry.placements.values())
            ...
            if (symbol.getAddress().equals(choice.getValue().resolve(p)))
              explicit.add(
                  SymbolFile.parse(
                      ...
                      .location());
        if (explicit.size() != 1) {
          diagnostics.add(symbol.getName() + ": physical identity unresolved");
          continue;
        }
        location = explicit.iterator().next();
      } else {
        var physical = identities.get(0);
        int bank = physical.bank();
        if (c != null && c.mapper() == Cartridge.Mapper.ROM_ONLY && physical.region().equals("ROM"))
          bank = 0;
        location =
            new SymbolFile.Location(
                physical.region().equals("BOOT") ? "BOOT" : "BANK",
                bank,
                (int) symbol.getAddress().getOffset());
      }
      ...
      if (validated.symbols().size() != 1) {
        diagnostics.add(symbol.getName() + ": name not representable by RGBDS grammar");
        continue;
      }
      if (seen.add(line)) out.add(entry);
    }
    out.sort(
        Comparator.comparing((SymbolFile.Symbol x) -> x.location().form())
            .thenComparingLong(x -> x.location().bank())
            .thenComparingInt(x -> x.location().address())
            .thenComparing(SymbolFile.Symbol::name));
