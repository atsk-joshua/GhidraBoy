...
public final class BankAnalysis {
    ...
    public record Finding(String source,String access,List<String> targets,String reason,AnalysisResult.Confidence confidence,int operation,int operand,int byteIndex) {
    ...
    public static AnalysisResult preview(Program p,Address start,MapperState assumption,AnalysisResult.Configuration configuration,TaskMonitor monitor) throws Exception {
        ...
        var candidates=new AnalysisCandidates(); var targets=candidates.targets; var reasons=candidates.reasons;
        int count=0;
        ...
        while(!queue.isEmpty()) {
            ...
            if(ins==null) { reasons.put(AnalysisCandidates.Site.control(w.address,"flow"),"No defined instruction; data/undefined bytes left intact"); continue; }
            if(!fetchEstablished(p,cartridge,w.state,ins)) {
                reasons.put(AnalysisCandidates.Site.control(w.address,"flow"),"Instruction fetch crosses an unestablished physical execution view"); continue;
            }
            ...
            for(var op:ins.getPcode()) {
                ...
                if(op.getOpcode()!=PcodeOp.BRANCH && op.getOpcode()!=PcodeOp.CBRANCH && op.getOpcode()!=PcodeOp.CALL && op.getOpcode()!=PcodeOp.RETURN)
                    for(int operand=0;operand<op.getNumInputs();operand++) {
                        ...
                        if(input.isAddress()) readAccess(p,cartridge,state,(int)(input.getOffset()&65535),input.getSize(),w.address,operationIndex,operand,targets,reasons);
                    }
                if(op.getOpcode()==PcodeOp.STORE) {
                    ...
                    if(internal || ptr==null) {
                        ...
                        unknownAccess(w.address,AnalysisCandidates.Access.WRITE,operationIndex,op.getInput(2).getSize(),reasons,"Unknown store may affect mapper state");
                    } else {
                        int cpu=(int)(ptr & 65535);
                        int width=op.getInput(2).getSize();
                        state=writeAccess(p,cartridge,state,cpu,width,val,w.address,operationIndex,targets,reasons);
                        changedMapper |= touchesMapper(cartridge,cpu,width);
                    }
                } else if(op.getOpcode()==PcodeOp.LOAD) {
                    Long ptr=value(op.getInput(1),regs,unique);
                    if(ptr!=null) readAccess(p,cartridge,state,(int)(ptr&65535),op.getOutput().getSize(),w.address,operationIndex,-1,targets,reasons);
                    else unknownAccess(w.address,AnalysisCandidates.Access.READ,operationIndex,op.getOutput().getSize(),reasons,"Unknown load address");
                }
                ...
                if(output!=null) {
                    ...
                    if(output.isAddress()) {
                        ...
                        state=writeAccess(p,cartridge,state,cpu,output.getSize(),result,w.address,operationIndex,targets,reasons);
            ...
            if(next==null && !ins.isFallThroughOverridden() && flow.hasFallthrough() && ins.getAddress().getOffset()+ins.getLength()>65535)
            ...
            if(next!=null) {
                ...
                if(nextViews.isEmpty()) reasons.put(AnalysisCandidates.Site.control(w.address,"flow"),"Fallthrough execution view unresolved after call, mapper write or window transition");
            ...
            if(flow.isComputed()) reasons.put(AnalysisCandidates.Site.control(w.address,"flow"),"Indirect flow requires a validated per-program convention");
        }
        } catch(ghidra.util.exception.CancelledException cancelled) {
            completion=AnalysisResult.Completion.CANCELLED;
        }
        if(completion==AnalysisResult.Completion.STATE_LIMIT) reasons.put(AnalysisCandidates.Site.control(start,"limit"),configuration.stateLimit()+"-state worklist bound reached");
        if(completion==AnalysisResult.Completion.CANCELLED) reasons.put(AnalysisCandidates.Site.control(start,"cancelled"),"Exploration cancelled");
        ...
        return new AnalysisResult(2,AnalysisResult.ENGINE_VERSION,List.of(start.toString()),assumption,configuration,completion,count,queue.size(),fingerprint,findings,
    ...
    private static boolean fetchEstablished(Program p,Cartridge c,MapperKnowledge state,ghidra.program.model.listing.Instruction ins) throws Exception {
        ...
        if(executionWindow((int)start)==executionWindow((int)(end&65535)) && end<=65535) return true;
    ...
    private static int executionWindow(int cpu) {
        int[] ends={0x4000,0x8000,0xa000,0xc000,0xd000,0xe000,0xf000,0xfe00,0xfea0,0xff00,0xff80,0xffff,0x10000};
        for(int i=0;i<ends.length;i++) if(cpu<ends[i]) return i;
        throw new IllegalArgumentException("CPU address out of range");
    }
    private static void unknownAccess(Address from,AnalysisCandidates.Access access,int operation,int width,
            Map<AnalysisCandidates.Site,String> reasons,String reason) {
        for(int i=0;i<width;i++) reasons.put(new AnalysisCandidates.Site(from,access,operation,-1,i),reason);
    }
    ...
    private static MapperKnowledge writeAccess(Program p,Cartridge c,MapperKnowledge state,int address,int width,Long value,Address from,int operation,
            Map<AnalysisCandidates.Site,Set<Address>> targets,Map<AnalysisCandidates.Site,String> reasons) throws Exception {
    ...
    private static void readAccess(Program p,Cartridge c,MapperKnowledge state,int address,int width,Address from,int operation,int operand,
            Map<AnalysisCandidates.Site,Set<Address>> targets,Map<AnalysisCandidates.Site,String> reasons) throws Exception {
        for(int i=0;i<width;i++) record(p,c,state,(address+i)&65535,false,from,operation,operand,i,targets,reasons);
    }
    ...
    private static void record(Program p,Cartridge c,MapperKnowledge s,int cpu,boolean write,Address from,int operation,int operand,int byteIndex,
                               Map<AnalysisCandidates.Site,Set<Address>> targets,Map<AnalysisCandidates.Site,String> reasons) throws Exception {
        var key=new AnalysisCandidates.Site(from,write?AnalysisCandidates.Access.WRITE:AnalysisCandidates.Access.READ,operation,operand,byteIndex);
