...
public final class MapperTopology {
  ...
  public static List<View> romViews(Cartridge cartridge) {
    var views =
        new TreeSet<View>(Comparator.comparingInt(View::bank).thenComparingInt(View::cpuWindow));
    int lowMax =
    ...
    int highMax =
        cartridge.mapper() == Cartridge.Mapper.MBC1
            ? 3
            : cartridge.mapper() == Cartridge.Mapper.MBC5 ? 1 : 0;
        ...
        for (int mode = 0; mode <= modeMax; mode++) {
          ...
          for (int window : new int[] {0, 0x4000}) {
            ...
            if (physical != null && physical.region().equals("ROM"))
              views.add(new View(physical.bank(), window));
          }
        }
      }
    }
