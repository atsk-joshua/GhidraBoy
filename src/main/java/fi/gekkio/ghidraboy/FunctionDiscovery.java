...
public final class FunctionDiscovery {
  ...
  public static List<String> discover(
      Program p,
      List<Address> declaredCode,
      List<BankAnalysis.Finding> findings,
      TaskMonitor monitor)
      throws Exception {
    // Legacy lists carry neither completeness nor dependency evidence. Only explicit seeds are
    // accepted.
    return discoverSeeds(p, declaredCode, List.of(), monitor);
  }

  public static List<String> discover(
      Program p, List<Address> declaredCode, AnalysisResult result, TaskMonitor monitor)
      throws Exception {
    ...
    if (saved != null) {
      ...
      if (!Objects.equals(active.assumption(), result.assumption())
          || !active.configuration().equals(result.configuration())
          || !active.starts().equals(result.starts()))
        throw new IllegalStateException(
            "Analysis assumptions changed; apply the current preview before function discovery");
    }
  ...
  private static List<String> discoverSeeds(
      Program p,
      List<Address> declaredCode,
      List<BankAnalysis.Finding> findings,
      TaskMonitor monitor)
    ...
    var entries = p.getSymbolTable().getExternalEntryPointIterator();
    while (entries.hasNext()) seeds.add(entries.next());
    for (var f : findings)
      if (f.access().equals("call")
          && f.targets().size() == 1
          && f.confidence() == AnalysisResult.Confidence.PROVEN) {
        var a = p.getAddressFactory().getAddress(f.targets().get(0));
        if (a != null) seeds.add(a);
      }
    ...
    try {
      ...
      for (var a : seeds) {
        ...
        if (p.getListing().getInstructionAt(a) == null
            || p.getListing().getDefinedDataContaining(a) != null) {
          result.add(a + ": no defined instruction; data/undefined bytes preserved");
          continue;
        }
        boolean userLabel = false;
        for (var symbol : p.getSymbolTable().getSymbols(a))
          if (symbol.getSource() != SourceType.DEFAULT) userLabel = true;
        if (userLabel) {
          result.add(a + ": user symbol preserved; use explicit function editing to promote it");
          continue;
        }
