...
public final class SymbolFile {
    ...
    private static final Pattern NAME=Pattern.compile("[A-Za-z_]([A-Za-z0-9_@#$.]|\\\\u[A-Fa-f0-9]{4}|\\\\U[A-Fa-f0-9]{8})*");
    private SymbolFile() { }
    public static Result parse(byte[] bytes) throws java.nio.charset.CharacterCodingException {
        ...
        var symbols=new ArrayList<Symbol>(); var warnings=new ArrayList<String>(); var seen=new HashSet<String>();
        int line=0;
        for(String raw:input.split("\\r?\\n",-1)) {
            ...
            if(tokens.length<2) { warnings.add(line+": reserved single-token line ignored"); continue; }
            try {
                String[] parts=tokens[0].split(":",-1);
                if(parts.length>2) throw new IllegalArgumentException("Invalid location form");
                String form=parts.length==1?"ANY":parts[0].equalsIgnoreCase("BOOT")?"BOOT":"BANK";
                long bank=form.equals("BANK")?hex(parts[0],0xffffffffL):0;
                int address=(int)hex(parts[parts.length-1],0xffff);
                String name=tokens[1];
                if(!NAME.matcher(name).matches()) throw new IllegalArgumentException("Invalid name/escape");
                ...
                for(int i=2;i<tokens.length;i++) if(!tokens[i].startsWith("@")) warnings.add(line+": unknown metadata "+tokens[i]);
            } catch(IllegalArgumentException e) { warnings.add(line+": "+e.getMessage()); }
        }
        return new Result(List.copyOf(symbols),List.copyOf(warnings));
    }
    private static long hex(String text,long max) {
        if(!text.matches("[A-Fa-f0-9]+")) throw new IllegalArgumentException("Invalid hexadecimal location");
    ...
    public static Optional<Symbol> parent(Symbol local,List<Symbol> all) {
        ...
        return all.stream().filter(s->s.name.equals(local.name.substring(0,dot)) &&
            s.location.form.equals(local.location.form) && s.location.bank==local.location.bank &&
            s.location.address<=local.location.address).max(Comparator.comparingInt(s->s.location.address));
    }
}
