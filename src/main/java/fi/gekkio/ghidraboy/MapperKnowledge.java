...
public record MapperKnowledge(Integer low, Integer high, Integer mode, Integer ram, Boolean enabled,
                              Integer vbk, Integer svbk, Integer latch) {
    public static MapperKnowledge unknown() { return new MapperKnowledge(null,null,null,null,null,null,null,null); }
    public static MapperKnowledge from(MapperState s) {
        return s==null?unknown():new MapperKnowledge(s.romLow(),s.romHigh(),s.mode(),s.ramSelect(),s.ramEnabled(),s.vbk(),s.svbk(),s.latch());
    }
    private MapperState representative() {
        return new MapperState(low==null?1:low,high==null?0:high,mode==null?0:mode,ram==null?0:ram,
            enabled!=null&&enabled,vbk==null?0:vbk,svbk==null?1:svbk,latch==null?0:latch);
    }
    public MapperKnowledge write(Cartridge c,int address,Integer value) {
        ...
        Integer l=low,h=high,m=mode,r=ram,v=vbk,w=svbk,t=latch; Boolean e=enabled;
        ...
        switch(c.mapper()) {
            case MBC1 -> {
                ...
                else if(address<0x6000) { h=value==null?null:s.romHigh(); r=value==null?null:s.ramSelect(); }
                else if(address<0x8000) m=value==null?null:s.mode();
            }
            case MBC2 -> {
                if(address<0x4000) {
                    if((address&0x100)==0) e=value==null?null:s.ramEnabled(); else l=value==null?null:s.romLow();
                }
            }
            case MBC3 -> {
                if(address<0x2000) e=value==null?null:s.ramEnabled();
    ...
    public MapperState.Resolution translate(Cartridge c,int cpu,boolean write) {
        ...
        else if(cpu>=0xa000 && cpu<0xc000 && c.mapper()!=Cartridge.Mapper.ROM_ONLY && c.mapper()!=Cartridge.Mapper.RAW) {
            missing=enabled==null;
            if(Boolean.TRUE.equals(enabled)) missing |= switch(c.mapper()) {
                case MBC1 -> mode==null || (mode==1 && high==null);
                case MBC3,MBC5 -> ram==null;
                default -> false;
            };
        } else if(c.color() && ((cpu>=0xd000 && cpu<0xe000) || (cpu>=0xf000 && cpu<0xfe00))) missing=svbk==null;
        if(missing) return MapperState.Resolution.other("unknown","Required mapper register is unknown");
        return MapperState.translate(c,representative(),cpu,write);
    }
    public boolean allowsExecution(Cartridge c,int cpu,int bank) {
        int lowMax=switch(c.mapper()) { case MBC1 -> 31; case MBC2 -> 15; case MBC3 -> 127; case MBC5 -> 255; default -> 0; };
        int highMax=c.mapper()==Cartridge.Mapper.MBC1?3:c.mapper()==Cartridge.Mapper.MBC5?1:0;
        for(int l=low==null?0:low;l<=(low==null?lowMax:low);l++)
            for(int h=high==null?0:high;h<=(high==null?highMax:high);h++)
                for(int m=mode==null?0:mode;m<=(mode==null?1:mode);m++) {
                    var physical=MapperState.translate(c,new MapperState(l,h,m,0,false,0,1,0),cpu,false).physical();
                    if(physical!=null && physical.region().equals("ROM") && physical.bank()==bank) return true;
                }
        return false;
    }
}
