  ...
  public static void apply(Program p, AnalysisResult result, TaskMonitor monitor) throws Exception {
    ...
    try {
      ...
      var priorFindings =
          prior.isJsonArray()
              ? prior
              : prior.isJsonObject() ? prior.getAsJsonObject().get("findings") : null;
      if (priorFindings != null && priorFindings.isJsonArray()) {
        var last = new LinkedHashMap<String, Finding>();
        for (var finding : ProgramMapping.JSON.fromJson(priorFindings, Finding[].class))
          last.put(finding.source(), finding);
        for (var finding : last.values()) {
          var address = p.getAddressFactory().getAddress(finding.source());
          var bookmark =
              address == null
                  ? null
                  : p.getBookmarkManager().getBookmark(address, "Analysis", "GhidraBoy");
          if (bookmark != null
              && bookmark
                  .getComment()
                  .equals(finding.access() + ": " + finding.reason() + " " + finding.targets()))
          ...
          owned.bookmark(
              p.getBookmarkManager()
                  .setBookmark(
                      ...
                      "Analysis",
                      category,
                      f.confidence() + ": " + f.reason() + " " + f.targets()));
      }
      record Proven(
          ghidra.program.model.address.Address from, ghidra.program.model.address.Address to) {}
      var requests = new LinkedHashMap<Proven, Set<String>>();
      if (result.complete())
        for (var finding : findings)
          if (finding.confidence() == AnalysisResult.Confidence.PROVEN
              && finding.targets().size() == 1) {
            var from = p.getAddressFactory().getAddress(finding.source());
            var to = p.getAddressFactory().getAddress(finding.targets().get(0));
            if (from == null || to == null)
              throw new IllegalArgumentException("Result contains an invalid static address");
            requests
                .computeIfAbsent(new Proven(from, to), ignored -> new HashSet<>())
                .add(finding.access());
          }
      for (var request : requests.entrySet()) {
        monitor.checkCancelled();
        var key = request.getKey();
        var access = request.getValue();
        boolean preserve = false;
        for (var reference : p.getReferenceManager().getReferencesFrom(key.from()))
          if (reference.getToAddress().equals(key.to())
              || (reference.getOperandIndex() == -1 && !reference.isMemoryReference()))
            preserve = true;
        ...
        var type =
            access.contains("read") && access.contains("write")
                ...
                : access.contains("write")
                    ? RefType.WRITE
                    : access.contains("read") ? RefType.READ : RefType.DATA;
        owned.reference(
