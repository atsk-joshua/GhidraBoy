...
public class Create1131Fixture extends GhidraScript {
  ...
  public void run() throws Exception {
    if (!ghidra.framework.Application.getApplicationVersion().equals("11.3.1"))
      throw new AssertionError("Actual 11.3.1 required");
    ...
    try {
      ...
      bytes[0x147] = 3;
      bytes[0x148] = 1;
      bytes[0x150] = (byte) 0xce;
      bytes[0x151] = 0;
      bytes[0x152] = (byte) 0xc9;
      ...
      var ns =
          p.getSymbolTable()
              .createNameSpace(p.getGlobalNamespace(), "UserNamespace", SourceType.USER_DEFINED);
      ...
      var type = new StructureDataType("UserType1131", 0);
      ...
      bank.setName("student bank 1131");
      ...
      try {
        ...
        if (emu.readRegister("A").intValue() != 0 || (emu.readRegister("F").intValue() & 16) != 0)
          throw new AssertionError("Expected original ADC defect on historical language");
      } finally {
        emu.dispose();
      }
