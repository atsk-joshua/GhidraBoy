...
public class Verify1131Upgrade extends GhidraScript {
    private void check(boolean ok,String what) { if(!ok) throw new AssertionError(what); }
    private void annotations() {
        var p=currentProgram; var a=toAddr(0x150); var f=p.getFunctionManager().getFunctionAt(a);
        check(f!=null && f.getName().equals("user_function") && f.getBody().getNumAddresses()==3,"function/body");
        check(f.hasCustomVariableStorage() && f.getReturn().getRegister().getName().equals("A"),"custom storage");
        check("keep comment 1131".equals(p.getListing().getComment(CodeUnit.EOL_COMMENT,a)),"comment");
        ...
        var labels=p.getSymbolTable().getSymbols("user_label"); boolean namespace=false;
        while(labels.hasNext()) if(labels.next().getName(true).equals("UserNamespace::user_label")) namespace=true;
        check(namespace,"namespaced label");
        var block=p.getMemory().getBlock("student bank 1131");
        try { check(block!=null && p.getMemory().getByte(block.getStart().add(0x12))==0x5a,"patch/renamed overlay"); }
    ...
    @Override public void run() throws Exception {
        ...
        check(ProgramMapping.cartridge(currentProgram).hardware().equals("GB"),"historical GB override must survive CGB header");
        LegacyEnhancement.enhance(currentProgram,"AUTO",monitor);
        annotations();
        ...
        try {
            ...
            check(emu.readRegister("A").intValue()==0 && (emu.readRegister("F").intValue()&16)!=0,"updated ADC p-code");
        } finally { emu.dispose(); }
        check(ProgramMapping.exportBytes(currentProgram,true,false,monitor)[0x8012]==0x5a,"export patch");
