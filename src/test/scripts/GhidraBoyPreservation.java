...
public class GhidraBoyPreservation extends GhidraScript {
    @Override public void run() throws Exception {
        if(getScriptArgs()[0].equals("create")) {
            ...
            try {
                byte[] rom=new byte[0x10000]; rom[0x147]=3; rom[0x148]=1; rom[0x149]=3;
                rom[0x150]=0x3e; rom[0x151]=0x12; rom[0x152]=(byte)0xc9;
                ...
                var type=new StructureDataType("UserType",0); type.add(ByteDataType.dataType,"owned",null);
                p.getDataTypeManager().addDataType(type,DataTypeConflictHandler.KEEP_HANDLER);
                var bank=p.getMemory().getBlock("rom2"); bank.setName("student bank");
            ...
            require(p.getFunctionManager().getFunctionAt(a).getName().equals("user_function"),"function");
            require(p.getFunctionManager().getFunctionAt(a).hasCustomVariableStorage(),"custom storage");
            require(p.getFunctionManager().getFunctionAt(a).getReturn().getRegister().getName().equals("A"),"return storage");
    ...
    private void require(boolean value,String what) { if(!value) throw new AssertionError(what); }
}
