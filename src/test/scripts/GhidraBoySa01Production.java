...
public class GhidraBoySa01Production extends GhidraScript {
  ...
  private ghidra.app.decompiler.DecompileResults verifyNative(DecompInterface decompiler,
    ...
    var function = currentProgram.getFunctionManager().getFunctionAt(source);
    if (function.isThunk() && function.getThunkedFunction(false).getEntryPoint().getAddressSpace().getName().startsWith(SoftwareCallExecutionView.PREFIX)) {
      // Canonical injection supplies its complete continuation CFG. Bounded listing flow
      // prevents native decoding of obsolete-bank bytes before that CFG is injected.
      var instruction = currentProgram.getListing().getInstructionAt(source);
      require(instruction.getFlowOverride() == FlowOverride.NONE && source.equals(instruction.getFallThrough()),
          "canonical complete injection owns bounded self-fallthrough transport");
      require(Arrays.stream(instruction.getPcode(false)).anyMatch(op -> op.getOpcode() == PcodeOp.CALL),
          "canonical raw architectural call stays intact");
      var alias = currentProgram.getListing().getInstructionAt(function.getThunkedFunction(false).getEntryPoint());
      require(alias.getFlowOverride() == FlowOverride.NONE && alias.getFallThrough() != null,
          "execution alias retains ordinary mapped continuation");
    }
  ...
  private void snapshotViews(Path work, String phase) throws Exception {
    var output = new TreeMap<String, List<String>>();
    for (var block : currentProgram.getMemory().getBlocks()) {
      String name = block.getStart().getAddressSpace().getName();
      if (!name.startsWith(SoftwareCallExecutionView.PREFIX)) continue;
      var fields = output.computeIfAbsent(name, ignored -> new ArrayList<>());
      fields.add("block:" + block.getStart() + ":" + block.getEnd() + ":" + block.getFlags() + ":" + block.getComment());
      for (var info : block.getSourceInfos()) fields.add("mapping:" + info.getMappedRange());
      for (var ins : currentProgram.getListing().getInstructions(new AddressSet(block.getStart(), block.getEnd()), true)) {
        fields.add("instruction:" + ins.getAddress() + ":" + ins.getLength() + ":" + ins.isLengthOverridden() + ":" + Arrays.toString(ins.getPcode(false)) + ":" + ins.getFlowOverride() + ":" + ins.isFallThroughOverridden() + ":" + ins.getFallThrough());
        for (var type : CommentType.values()) fields.add("comment:" + ins.getAddress() + ":" + type + ":" + ins.getComment(type));
      }
    }
    for (var f : currentProgram.getFunctionManager().getFunctions(true)) {
      var fields = output.get(f.getEntryPoint().getAddressSpace().getName());
      if (fields == null) continue;
      var frame = f.getStackFrame(); var ret = f.getReturn(); var storage = ret.getVariableStorage();
      fields.add("function:" + f.getEntryPoint() + ":" + f.getName(true) + ":" + f.getBody() + ":" + f.getSignatureSource() + ":" + f.getCallingConventionName() + ":" + f.hasCustomVariableStorage() + ":" + f.getComment() + ":" + f.getRepeatableComment() + ":" + f.isInline() + ":" + f.hasNoReturn() + ":" + f.hasVarArgs() + ":" + f.getCallFixup() + ":" + f.getStackPurgeSize());
      fields.add("stack:" + f.getEntryPoint() + ":" + frame.getLocalSize() + ":" + frame.getReturnAddressOffset() + ":" + frame.getParameterOffset() + ":" + frame.getParameterSize() + ":" + frame.growsNegative());
      fields.add("return:" + f.getEntryPoint() + ":" + ret.getSource() + ":" + ret.getName() + ":" + ret.getComment() + ":" + ret.getFirstUseOffset() + ":" + ret.getFormalDataType().getClass().getName() + ":" + ret.isForcedIndirect() + ":" + storage.getSerializationString() + ":" + storage.isForcedIndirect() + ":" + storage.isAutoStorage() + ":" + storage.getAutoParameterType());
      for (var v : f.getAllVariables()) fields.add("variable:" + f.getEntryPoint() + ":" + v.getName() + ":" + v.getSource() + ":" + v.getDataType() + ":" + v.getVariableStorage());
    }
    for (var symbol : currentProgram.getSymbolTable().getAllSymbols(true)) {
      var fields = output.get(symbol.getAddress().getAddressSpace().getName());
      if (fields != null && !symbol.isDynamic()) fields.add("symbol:" + symbol.getID() + ":" + symbol.getAddress() + ":" + symbol.getName(true) + ":" + symbol.getSource() + ":" + symbol.isPinned());
    }
    var sources = currentProgram.getReferenceManager().getReferenceSourceIterator(currentProgram.getMemory(), true);
    while (sources.hasNext()) for (var ref : currentProgram.getReferenceManager().getReferencesFrom(sources.next())) {
      for (var entry : output.entrySet()) {
        if (!ref.getFromAddress().getAddressSpace().getName().equals(entry.getKey()) && !ref.getToAddress().getAddressSpace().getName().equals(entry.getKey())) continue;
        if (!ref.getReferenceType().isFlow() && !ref.getReferenceType().isOverride() && ref.getSource() == SourceType.ANALYSIS) continue;
        entry.getValue().add("reference:" + ref.getFromAddress() + ":" + ref.getToAddress() + ":" + ref.getReferenceType() + ":" + ref.getOperandIndex() + ":" + ref.getSource() + ":" + ref.isPrimary() + ":" + ref.getSymbolID());
      }
    }
    var bookmarks = currentProgram.getBookmarkManager().getBookmarksIterator();
    while (bookmarks.hasNext()) { var b = bookmarks.next(); var fields = output.get(b.getAddress().getAddressSpace().getName());
      if (fields != null) fields.add("bookmark:" + b.getAddress() + ":" + b.getTypeString() + ":" + b.getCategory() + ":" + b.getComment()); }
    for (var register : currentProgram.getProgramContext().getRegisters())
      for (var range : currentProgram.getProgramContext().getRegisterValueAddressRanges(register)) {
        var fields = output.get(range.getMinAddress().getAddressSpace().getName());
        if (fields != null) fields.add("context:" + register.getName() + ":" + range + ":" + currentProgram.getProgramContext().getRegisterValue(register, range.getMinAddress()));
      }
    for (var fields : output.values()) Collections.sort(fields);
    Files.writeString(work.resolve("view-state-" + phase + ".json"), ProgramMapping.JSON.toJson(output));
  }
  ...
  @Override public void run() throws Exception {
    ...
    if (mode.equals("prepare")) {
      // Retain the prepared lifecycle matrix, while separately proving that its first two
      // configured roots now produce a read-only missing-instruction discovery plan.
      require(currentProgram.getListing().getInstructionAt(toAddr(0x200)) == null, "fresh helper instruction absent");
      var fresh = SoftwareCallApplication.preview(currentProgram, configurations().subList(0, 2), monitor);
      require(!fresh.instructionDiscovery().candidates().isEmpty(), "fresh rooted discovery inventory");
      require(currentProgram.getListing().getInstructionAt(toAddr(0x200)) == null, "preview leaves helper undefined");
      Files.writeString(work.resolve("fresh-discovery-preview.json"), ProgramMapping.JSON.toJson(fresh.instructionDiscovery()));
    ...
    verifyNestedRepairInventory(work);
    snapshotViews(work, Files.exists(work.resolve("reapplied-inventory.json")) ? "reapplied-analyzed" : "analyzed");
