...
class AnalysisHardeningTest : IntegrationTest() {
    private fun program(banks: Int = 4, prepare: (ByteArray) -> Unit, test: (ProgramDB) -> Unit) {
        val bytes = ByteArray(banks * 0x4000)
        bytes[0x143] = 0x80.toByte(); bytes[0x147] = 0x1b; bytes[0x148] = Integer.numberOfTrailingZeros(banks / 2).toByte(); bytes[0x149] = 3
        prepare(bytes)
        val consumer = Any()
        ...
        try {
            ...
            p.withTransaction {
                ...
                d.setRepeatPatternLimitIgnored(range); d.disassemble(address(0x150), range)
            }
            test(p)
    ...
    fun `both branch orders preserve candidates without proof on worklist exhaustion`() {
        for (reverse in listOf(false, true)) program(prepare = { b ->
            ...
            val r = BankAnalysis.preview(p, address(0x150), MapperState.reset(), AnalysisResult.Configuration(4096, reverse), TaskMonitor.DUMMY)
    ...
    @Test
    fun `loop duplicate states do not consume the exploration budget`() = program(prepare = { b ->
    ...
    @Test
    fun `cancelled preview and patched results cannot mutate the program`() = program(prepare = { b ->
        byteArrayOf(0, 0, 0, 0xc3.toByte(), 0, 0x40).copyInto(b, 0x150)
    }) { p ->
        val monitor = object : TaskMonitorAdapter(true) {
            var exploring = false
            var checks = 0
            override fun setMessage(message: String?) { if (message == "Exploring bank states") exploring = true }
            override fun checkCancelled() { if (exploring && ++checks == 3) throw CancelledException() }
        }
        ...
        p.withTransaction { p.listing.clearCodeUnits(address(0x151), address(0x151), false); p.memory.setByte(address(0x151), 0x3f) }
    ...
    fun `wide writes across mapper hardware and address boundaries affect every byte`() {
        data class Case(val banks: Int, val where: Int, val word: Int, val target: Int, val expected: String)
        for (case in listOf(Case(4, 0x1fff, 0x020a, 0x4000, "rom2::4000"), Case(512, 0x2fff, 0x0100, 0x4000, "rom256::4000"),
            Case(4, 0xff4e, 0x0100, 0x8034, "vram1::8034"), Case(4, 0xff6f, 0x0300, 0xd034, "wram3::d034"), Case(4, 0xffff, 0x0a00, 0xa010, "a010"))) {
            program(case.banks, prepare = { b ->
                byteArrayOf(0x31, case.word.toByte(), (case.word shr 8).toByte(), 8, case.where.toByte(), (case.where shr 8).toByte(),
                    0xfa.toByte(), case.target.toByte(), (case.target shr 8).toByte(), 0xc9.toByte()).copyInto(b, 0x150)
            }) { p ->
                val r = BankAnalysis.preview(p, address(0x150), MapperState.reset(), AnalysisResult.Configuration.DEFAULT, TaskMonitor.DUMMY)
                assertTrue(r.findings().any { it.source() == "0156" && it.access() == "read" && case.expected in it.targets() }, "$case: $r")
            }
        }
    }
    @Test
