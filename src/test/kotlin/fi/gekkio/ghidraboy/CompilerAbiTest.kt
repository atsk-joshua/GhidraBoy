...
class CompilerAbiTest : IntegrationTest() {
    ...
    fun `per function profiles cover emitted ordinary variadic banked and explicit aggregate storage`() {
        ...
        try {
            val parameters = listOf(CompilerAbi.Parameter("a", "u8", null), CompilerAbi.Parameter("b", "u16", null), CompilerAbi.Parameter("c", "u8", null))
            ...
            val f = p.withTransaction { p.functionManager.createFunction("owned_name", address(0x100), AddressSet(address(0x100)), SourceType.USER_DEFINED) }
            ...
            assertTrue(variadic.variadic()); assertTrue(variadic.parameters()[0].storage().startsWith("Stack[0x2]")); assertEquals(0, variadic.stackPurgeBytes())
            val banked = CompilerAbi.preview(p, CompilerAbi.Request("sdcc451-banked-callee", "u8", null, parameters.take(2), null, null))
            assertTrue(banked.parameters()[0].storage().startsWith("Stack[0x6]"), banked.toString()); assertTrue(banked.parameters()[1].storage().startsWith("Stack[0x7]"), banked.toString())
            val aggregate = CompilerAbi.Request("explicit", "void", null, listOf(CompilerAbi.Parameter("result", "ptr", "stack:2"), CompilerAbi.Parameter("value", "bytes:3", "stack:4")), 5, "__sdcc451_call1_first16")
            ...
            assertThrows(IllegalArgumentException::class.java) { CompilerAbi.preview(p, CompilerAbi.Request("sdcc451-call1", "bytes:3", null, parameters, null, null)) }
        } finally { p.release(consumer) }
    }
    @Test
    fun `decompiler honors per function preserved BC across a call`() {
        val consumer = Any(); val p = ProgramDB("ABI decompiler", language, language.defaultCompilerSpec, consumer)
        ...
        try {
            p.withTransaction {
                ...
                Disassembler.getDisassembler(p, TaskMonitor.DUMMY, null).disassemble(address(0x100), AddressSet(address(0x100), address(0x201)))
            }
            val caller = p.withTransaction { p.functionManager.createFunction("caller", address(0x100), AddressSet(address(0x100), address(0x108)), SourceType.USER_DEFINED) }
            val callee = p.withTransaction { p.functionManager.createFunction("preserved", address(0x200), AddressSet(address(0x200), address(0x201)), SourceType.USER_DEFINED) }
            CompilerAbi.apply(p, caller, CompilerAbi.Request("sdcc451-call1", "u8", null, listOf(), null, null))
            CompilerAbi.apply(p, callee, CompilerAbi.Request("sdcc451-preserves-bc", "u8", null, listOf(CompilerAbi.Parameter("a", "u8", null)), null, null))
        ...
        } finally { decompiler.dispose(); p.release(consumer) }
    }
    @Test
