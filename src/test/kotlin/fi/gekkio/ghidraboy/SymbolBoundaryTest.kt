...
class SymbolBoundaryTest : IntegrationTest() {
    ...
    fun `explicit end markers retain overlay space through rename reload and export`() {
        val consumer = Any(); val p = ProgramDB("boundary symbols", language, language.defaultCompilerSpec, consumer)
        try {
            val bytes = ByteArray(0x10000).also { it[0x147] = 0x1b; it[0x148] = 1; it[0x149] = 3 }
            ...
            assertEquals("vram0::a000", p.symbolTable.getSymbols("VramEnd").next().address.toString())
            ...
            assertEquals("renamed_vram::a000", p.symbolTable.getSymbols("VramEnd").next().address.toString())
            assertTrue(SymbolService.exportSymbols(p, TaskMonitor.DUMMY).text().contains("0:a000 VramEnd"))
            val input = directory.resolve("image.gb"); Files.write(input, bytes)
            ...
            assertTrue(SymbolService.companion(p).isEmpty)
            val companion = directory.resolve("image.sym"); Files.writeString(companion, "0:0100 Right\n")
