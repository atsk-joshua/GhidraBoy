...
class SymbolOwnershipTest : IntegrationTest() {
    private fun program(test: (ProgramDB) -> Unit) {
        ...
        try {
            val bytes = ByteArray(0x10000).also { it[0x147] = 0x19; it[0x148] = 1 }
    ...
    private fun put(p: ProgramDB, source: String, text: String = "2:4010 Shared\n") =
    ...
    fun `both source removal orders and changed reloads retain shared membership`() {
        for (first in listOf("a", "b")) program { p ->
            put(p, "a"); put(p, "b")
    ...
    fun `preexisting edited moved and promoted labels survive last source removal`() {
        for (change in listOf("preexisting", "rename", "namespace", "promotion")) program { p ->
            ...
            p.withTransaction {
                when (change) {
                    ...
                    "namespace" -> symbol.setNamespace(p.symbolTable.createNameSpace(p.globalNamespace, "Moved", SourceType.USER_DEFINED))
    ...
    @Test
    fun `cancelled import rolls back claims and newly created labels`() = program { p ->
        put(p, "a")
        val monitor = object : TaskMonitorAdapter(true) {
            var checks = 0
            override fun checkCancelled() { if (++checks == 3) throw CancelledException() }
        }
