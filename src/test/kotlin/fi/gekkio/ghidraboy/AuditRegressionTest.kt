...
class AuditRegressionTest : IntegrationTest() {
        ...
        try {
            ByteArrayProvider(bytes).use {
                CartridgeLayout.load(
                    p,
                    it,
                    if (boot) "CGB_BOOT" else "CARTRIDGE",
                    "AUTO",
                    GameBoyKind.CGB,
                    true,
                    true,
    ...
    @Test
    fun `R1 incomplete exploration cannot apply a singleton`() =
        loaded(prepare = { b ->
    ...
    @Test
    fun `R2 word store writes both mapper registers`() =
        loaded(prepare = { b ->
    ...
    @Test
    fun `R3 masked MBC1 bank zero has an upper window view`() =
        loaded(16, 3) { p ->
    ...
    @Test
    fun `R4 fixed to switchable window fallthrough resolves the new view`() =
        loaded(prepare = { b ->
    ...
    @Test
    fun `R5 shared symbol remains until the last source is removed`() =
        loaded { p ->
    ...
    @Test
    fun `R6 BOOT symbols never leak into echo RAM`() =
        loaded(boot = true) { p ->
