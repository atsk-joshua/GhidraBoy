...
class AnalysisBoundaryTest : IntegrationTest() {
    private fun program(test: (ProgramDB) -> Unit) {
        ...
        val consumer = Any()
        val p = ProgramDB("boundaries", language, language.defaultCompilerSpec, consumer)
        try {
    ...
    fun `relative cross-window instruction fetch requires the correct physical bytes`() {
        for (valid in listOf(false, true)) {
            program { p ->
                ...
                p.withTransaction {
                    ...
                    p.memory
                        .createByteMappedBlock(
                            "fetch suffix",
                            start.addressSpace.getAddressInThisSpaceOnly(0x8000),
                            source,
                            2,
                            false,
                        ).isExecute =
                ...
                assertTrue(
                    p.listing.getInstructionAt(start) != null,
                    "valid=$valid data=${p.listing.getCodeUnitAt(
                        start,
                    )} bytes=${p.memory.getByte(start)},${p.memory.getByte(start.add(1))}",
                )
                val result = BankAnalysis.preview(p, start, MapperState.reset(), AnalysisResult.Configuration.DEFAULT, TaskMonitor.DUMMY)
                if (valid) {
                    assertTrue(result.findings().any { it.access() == "jump" && "vram0::8001" in it.targets() }, result.toString())
                } else {
                    assertTrue(result.findings().any { it.reason().contains("Instruction fetch") }, result.toString())
                }
            }
        }
    }
    ...
    @Test
    fun `PC wrap follows the established low ROM window`() =
        program { p ->
            p.withTransaction {
