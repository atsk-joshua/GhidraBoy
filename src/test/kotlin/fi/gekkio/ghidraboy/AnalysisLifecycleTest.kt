...
class AnalysisLifecycleTest : IntegrationTest() {
    private fun program(test: (ProgramDB) -> Unit) {
        ...
        bytes[0x147] = 0x13; bytes[0x148] = 1; bytes[0x149] = 3; bytes[0x143] = 0x80.toByte()
    ...
    @Test
    fun `partial MBC3 ROM selection does not invent unrelated hardware state`() = program { p ->
        val r = BankAnalysis.preview(p, address(0x150), null, AnalysisResult.Configuration.DEFAULT, TaskMonitor.DUMMY)
        assertTrue(r.findings().any { it.access() == "call" && it.targets() == listOf("rom2::4000") && it.confidence() == AnalysisResult.Confidence.PROVEN }, r.toString())
    ...
    @Test
    fun `apply reapply remove cleans unchanged additions and preserves edits`() = program { p ->
        val r = BankAnalysis.preview(p, address(0x150), null, AnalysisResult.Configuration.DEFAULT, TaskMonitor.DUMMY)
        ...
        p.withTransaction {
            ...
            p.bookmarkManager.getBookmarksIterator("Analysis").next().set("User", "Edited bookmark")
        }
        AnalysisOwnership.removeAll(p, TaskMonitor.DUMMY)
    ...
    @Test
    fun `far convention rejects switchable callers and restores only owned overrides`() = program { p ->
        p.withTransaction {
            p.memory.setBytes(address(0x28), java.util.HexFormat.of().parseHex(FarCallConvention.SUPPORTED_BODY))
