...
public class GhidraBoyInstalledCheck extends GhidraScript {
  ...
  public void run() throws Exception {
    ...
    if (!p.getExecutableFormat().equals("Game Boy"))
      throw new AssertionError("Loader discovery: " + p.getExecutableFormat());
    var header = p.getListing().getDataAt(toAddr(0x134));
    var checksum = header.getComponentContaining(0x1a);
    if (((Scalar) checksum.getValue()).getUnsignedValue() != 0x1234
        || EndianSettingsDefinition.DEF.getChoice(checksum) != EndianSettingsDefinition.BIG)
    ...
    if (!java.util.Arrays.equals(original, current))
      throw new AssertionError("Untouched round trip");
    var parsed =
        SymbolFile.parse(
            "2:4010 Label\n2:4010 Label\n2:4011 Label.local\n4002 AnyBank\n"
    ...
    if (!SymbolService.exportRetained(p, "synthetic.sym").contains("Label.local"))
      throw new AssertionError("Retained symbols");
