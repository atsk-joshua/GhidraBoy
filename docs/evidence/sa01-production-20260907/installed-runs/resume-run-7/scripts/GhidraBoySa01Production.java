...
public class GhidraBoySa01Production extends GhidraScript {
  ...
  private ghidra.app.decompiler.DecompileResults verifyNative(DecompInterface decompiler,
    ...
    if (function.isThunk() && function.getThunkedFunction(false).getEntryPoint().getAddressSpace().getName().startsWith(SoftwareCallExecutionView.PREFIX)) {
      // Canonical injection supplies its complete continuation CFG. Terminal listing flow
      ...
      require(instruction.getFlowOverride() == FlowOverride.CALL_RETURN && instruction.getFallThrough() == null,
          "canonical complete injection owns terminal listing transport");
