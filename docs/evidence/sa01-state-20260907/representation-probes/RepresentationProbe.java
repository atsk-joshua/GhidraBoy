...
public class RepresentationProbe {
  ...
  public static void main(String[] args)throws Exception {
    var layout=new GhidraApplicationLayout(new File("/private/tmp/ghidraboy-sa00/ghidra_12.1.3_PUBLIC")) {
      @Override protected Map<String,GModule> findGhidraModules()throws IOException {var result=new HashMap<String,GModule>(super.findGhidraModules());result.put("RepresentationProbe",new GModule(getApplicationRootDirs(),new ResourceFile(ROOT+"/module")));return result;}
    };
    Application.initializeApplication(layout,new HeadlessGhidraApplicationConfiguration());
    var project=args.length==0?GhidraProject.createProject(ROOT,"segmented-probe-2",false):GhidraProject.openProject(ROOT,"segmented-probe-2",false);
    try{
