...
public class HookProbe {
  ...
  public static void main(String[] args)throws Exception {
    var layout=new GhidraApplicationLayout(new File("/private/tmp/ghidraboy-sa00/ghidra_12.1.3_PUBLIC")) {
      @Override protected Map<String,GModule> findGhidraModules()throws IOException {var result=new HashMap<String,GModule>(super.findGhidraModules());result.put("HookProbe",new GModule(getApplicationRootDirs(),new ResourceFile(ROOT+"/hook-module")));return result;}
    };
    Application.initializeApplication(layout,new HeadlessGhidraApplicationConfiguration());
    var project=args.length==0?GhidraProject.createProject(ROOT,"entry-hook-probe",false):GhidraProject.openProject(ROOT,"entry-hook-probe",false);
    try{
    ...
    for(var entry:cases.entrySet()){
      ...
      p.getListing().getInstructionAt(space.getAddress(0x24100)).setFallThrough(null);
