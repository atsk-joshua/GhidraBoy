...
public class FallthroughEntryProbe {
  ...
  public static void main(String[] args)throws Exception {
    var layout=new GhidraApplicationLayout(new File("/private/tmp/ghidraboy-sa00/ghidra_12.1.3_PUBLIC")) {
      @Override protected Map<String,GModule> findGhidraModules()throws IOException {var result=new HashMap<String,GModule>(super.findGhidraModules());result.put("FallthroughEntryProbe",new GModule(getApplicationRootDirs(),new ResourceFile(ROOT+"/entry-negative-Fallthrough")));result.put("Decompiler",new GModule(getApplicationRootDirs(),new ResourceFile(ROOT+"/native-dist/Ghidra/Features/Decompiler")));return result;}
    };
    Application.initializeApplication(layout,new HeadlessGhidraApplicationConfiguration());
    var project=args.length==0?GhidraProject.createProject(ROOT,"native-entry-negative-Fallthrough",false):GhidraProject.openProject(ROOT,"native-entry-negative-Fallthrough",false);
    try{
    ...
    for(var entry:cases.entrySet()){
      ...
      p.getFunctionManager().createFunction(entry.getKey(),banks.get(2).getAddress(0x4100),body,SourceType.USER_DEFINED);p.getFunctionManager().getFunctionAt(banks.get(2).getAddress(0x4100)).setCallingConvention("__ghidraboy_state_entry_v1");p.endTransaction(tx,true);
